...
import { TClassName } from '@/app/types/common';
...
export interface IInfiniteScrollProps {
    ...
    onScrollEventHandler?: (event: React.UIEvent<HTMLDivElement>) => void;
}
...
const InfiniteScroll = forwardRef<HTMLDivElement | null, IInfiniteScrollProps>(
    (
        ...
        }, [container, preventFixBottomScrollRef, hasMore, preventScrollItemBottom]);
        ...
        const renderSkeleton = () => {
            return (
                <div className={`${loadingSpiner && 'flex justify-center'}`}>
                    {(loadingSpiner || loadingSkeleton) && skeleton ? skeleton : <div>Loading...</div>}
                </div>
            );
        };
        ...
        useEffect(() => {
            ...
            if (hasMore && !isLoading) {
                ...
                const optionsObserver: { root: HTMLDivElement | null; rootMargin: string } = {
                    root: null,
                    rootMargin: '80px', //observer triggers when target is 50px from the bottom
                };
                if (hasMaxHeight) optionsObserver.root = container.current;
        ...
        }, [hasMore, isLoading, hasMaxHeight, fetchMore]);
